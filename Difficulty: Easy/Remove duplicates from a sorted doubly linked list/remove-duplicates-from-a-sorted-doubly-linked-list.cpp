...
public:
    ...
    Node * removeDuplicates(struct Node *head)
    {
        ...
        while (curr != nullptr)
        {
            ...
            // Move nextDistinct to the next distinct element
            while (nextDistinct != nullptr && nextDistinct->data == curr->data)
            {
                nextDistinct = nextDistinct->next;
            }
            
            // Remove duplicates by linking curr to the next distinct node
            curr->next = nextDistinct;
            
            // If nextDistinct is not null, update its prev pointer
